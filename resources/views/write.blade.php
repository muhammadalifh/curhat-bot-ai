            ...
            <div class="max-w-6xl w-full mx-auto sm:px-6 lg:px-8 space-y-4 py-4">
                ...
                <div class="w-full rounded-md bg-white border-2 border-gray-600 p-4 min-h-[60px] h-full text-gray-600">
                    <form action="/write/generate" method="post" class="inline-flex gap-2 w-full">
                        @csrf
                        <input required name="title" class="w-full outline-none text-2xl font-bold" placeholder="Coba Ketikkan Sesuatu..." />
                        <button type="submit"></button>
                    </form>
                </div>
                <div class="w-full rounded-md bg-white border-2 border-gray-600 p-4 min-h-[360px] h-full text-gray-600">
                    <textarea class="min-h-[360px] h-full w-full outline-none" spellcheck="false" style="font-size: 2rem;">{{ $content }}</textarea>
                </div>
            </div>
        </div>
    </body>
